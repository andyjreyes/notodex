...
//  FGallery
//
//  Modified and adapted by Andy Reyes on 10/25/12.
//  Created by Grant Davis on 5/19/10.
//  Grant's original code here: https://github.com/gdavis/FGallery-iPhone
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	if((self = [super initWithNibName:nil bundle:nil])) {
        ...
        _prevNavbarTranslucence             = self.navigationController.navigationBar.translucent;
        _hideTitle                          = NO;
...
- (void)loadView
{
    ...
    _captionContainer.exclusiveTouch			= NO;
    ...
    _caption.userInteractionEnabled = YES;
    ...
    _thumbsView.showsVerticalScrollIndicator    = NO;
    _thumbsView.showsHorizontalScrollIndicator  = NO;
    
	// set view
	self.view                                   = _container;
    
    // make navigation bar translucent if there is one
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    self.navigationController.navigationBar.translucent = _prevNavbarTranslucence;
}
...
- (void)enterFullscreen
{
    if (!_isThumbViewShowing)
    {
        ...
        if ([application respondsToSelector: @selector(setStatusBarHidden:withAnimation:)]) {
            [[UIApplication sharedApplication] setStatusBarHidden: YES withAnimation: UIStatusBarAnimationSlide]; // 3.2+
        } else {
...
- (void)didTapPhotoView:(FGalleryPhotoView*)photoView
{
    ...
    //Note: _isScrolling is never NO if all animations are on
	...
	if([_photoSource numberOfPhotosForPhotoGallery:self] > 0 )
	{
		if([_photoSource respondsToSelector:@selector(photoGallery:captionForPhotoAtIndex:)])
		{
			...
			if([caption length] > 0 )
			{
                ...
                _caption.numberOfLines = 0; //No line restrictions for captions
...
- (void)handleThumbClick:(id)sender
{
	...
	[self gotoImageByIndex:photoView.tag animated:YES]; //NO if tons of images
}
